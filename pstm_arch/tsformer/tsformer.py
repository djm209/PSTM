...
from .transformer_layers import TransformerLayers, FITS
...
class TSFormer(nn.Module):
    ...
    def __init__(self, patch_size, in_channel, embed_dim, num_heads, mlp_ratio, dropout, num_token, mask_ratio, encoder_depth, decoder_depth, individual, enc_in, cut_freq, mode="pre-train"):
        super().__init__()
        ...
        self.decoder = FITS(num_token, num_token*mask_ratio, individual, enc_in, cut_freq)
    ...
    def decoding(self, hidden_states_unmasked, masked_token_index):
        ...
        # hidden_states = self.decoder(hidden_states_unmasked)

        # hidden_states_masked = hidden_states[:, :, masked_token_index, :]
